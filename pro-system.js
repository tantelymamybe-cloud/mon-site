...
async function verifyCheckoutSession(clientRef) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      if (data.valid && data.token) {
        ...
        updateProUI();
        showProToast('🎉 Paiement confirmé — bienvenue en PRO !');
        return;
      }

      if (!data.pending) {
        // référence inconnue/expirée : inutile de continuer à réessayer
        showProToast("Le paiement n'a pas pu être confirmé. Contactez-nous si le prélèvement a bien eu lieu.");
        return;
      }
  ...
  showProToast("La confirmation prend plus de temps que prévu. Rechargez la page dans une minute — si le paiement a bien été débité, l'accès PRO s'activera automatiquement.");
}
...
async function goToProCheckout() {
  ...
  try {
    ...
    alert('Impossible de contacter le serveur de paiement pour le moment. Réessayez dans un instant.');
...
async function manageSubscription() {
  ...
  try {
    ...
    else { alert("Impossible d'ouvrir la gestion de l'abonnement pour le moment."); }
  } catch (e) {
    alert('Connexion au serveur impossible pour le moment.');
  }
}
...
function openProModal(toolId) {
  ...
  if (sub) {
    sub.textContent = (toolId && TOOL_LABELS[toolId])
      ? 'Vous avez atteint la limite gratuite du jour pour « ' + TOOL_LABELS[toolId] + ' ». Passez en PRO pour un usage illimité.'
      : 'Débloquez un usage illimité de tous les outils, sans limite quotidienne.';
  }
...
function renderUsageBanners() {
  ...
  document.querySelectorAll('.panel[id^="panel-"]').forEach((panel) => {
    ...
    if (pro) {
      banner.innerHTML = '<span class="pro-chip">✨ PRO</span> Utilisations illimitées sur cet outil.';
      return;
    }
    ...
    if (left > 0) {
      banner.innerHTML = left + ' utilisation' + (left > 1 ? 's' : '') +
        ' gratuite' + (left > 1 ? 's' : '') + ' restante' + (left > 1 ? 's' : '') +
        " aujourd'hui pour cet outil · " +
        '<a href="javascript:void(0)" onclick="openProModal(\'' + toolId + '\')">Passer en PRO</a>';
    } else {
      banner.classList.add('limit-reached');
      banner.innerHTML = 'Limite gratuite du jour atteinte pour cet outil · ' +
        '<a href="javascript:void(0)" onclick="openProModal(\'' + toolId + '\')">Passer en PRO pour continuer</a>';
    }
  });
}
